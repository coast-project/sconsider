...
class TestfwTranformerStateMachineTest(unittest.TestCase):
    ...
    def testParserFailedFailStartFailure(self):
        when(self.failed).failStartFailure(line=any(), testcase=any(), message=any(), cause=any()).thenReturn(None)
        self.parser.setState('failed')
        line = "21) SystemTest: File.cpp:123: this is the cause"
        self.parser.parseLine(line)
        verify(self.failed).failStartFailure(line=line, testcase="SystemTest", message="File.cpp:123: this is the cause", cause="this is the cause")
