import os, unittest, WorkingSetWriter, shutil, SomeUtils, tempfile
...
class TestProjectFuncs(unittest.TestCase):
    ...
    def testDetermineProjectFilePathNoProject(self):
        self.assertEqual(self.topproject, WorkingSetWriter.determineProjectFilePath(self.noprojectdir, self.tempdir))
        self.assertEqual(self.topproject, WorkingSetWriter.determineProjectFilePath(self.noprojecteitherdir, self.tempdir))
    
    def testGetProjectNameFromProjectFile(self):
        self.assertEqual('Top', WorkingSetWriter.getProjectNameFromProjectFile(self.topproject))
        self.assertEqual('Bottom', WorkingSetWriter.getProjectNameFromProjectFile(self.bottomproject))
        self.assertEqual(None, WorkingSetWriter.getProjectNameFromProjectFile(os.path.join(self.noprojectdir, '.project')))
        self.assertEqual(None, WorkingSetWriter.getProjectNameFromProjectFile(self.invalidprojectdesc))
