...
replaceTestFramework = (re.compile(r'(\b(TestFramework)\b::)'),
                        lambda mo: 'testframework::')

replaceSysLog = (re.compile(r'(\b(SysLog)\b::)'), lambda mo: 'SystemLog::')

replaceSysLogIncludes = (re.compile(r'include\s*.SysLog\.h.'),
                         'include "SystemLog.h"')
...
adjustCloneFunctionDef = (
    re.compile(r'^([ \t]*)(IFAObject\s*\*\s*(\w+::)?Clone\()(\)\s*const)',
               re.M),
...
changeToTestCaseType = (re.compile(
    r'(TString\s+)\bname\b(\s*\)\s*:\s*TestCaseType\()\bname\b',
    re.M), lambda mo: str(mo.group(1)) + 'tname' + str(mo.group(2)) + 'tname')

replaceMetathing1 = (re.compile(r'MetaThing\(\s*\)'),
                     lambda mo: 'Anything(Anything::ArrayMarker())')
...
replaceMetathing4 = (re.compile(r'MetaThing([^(])'),
                     lambda mo: 'Anything' + str(mo.group(1)))
...
putAnyMapperReplacements = (re.compile(
...
adjustUniqueIdGen = (re.compile(r'UniqueIdGen::GetUniqueId'),
...
replaceStringNposForCharacterFindFunctionsFailure = (
    re.compile(
        ...
        r'\s*(?P<comparison><\s*0|==\s*-1L?)', re.M),
    lambda mo: str(mo.group('function')) + str(mo.group('fnargs')) + '== String::npos'
)

replaceStringNposForCharacterFindFunctionsSuccess = (
    re.compile(
        r'^(?P<function>Contains|FirstCharOf|ContainsCharAbove|LastCharOf|StrChr|StrRChr)(?P<fnargs>\([^)]*\))'
        r'\s*(?P<comparison>!=\s*-1L?|>=?\s*0L?)', re.M),
    lambda mo: str(mo.group('function')) + str(mo.group('fnargs')) + '!= String::npos'
)
...
def extendReplaceFuncMap(extensionToReplaceFuncMap):
    extensionToReplaceFuncMap[reAny] = [
        replaceDoCheckStores, replaceClientInfoLookups,
        replaceRequestReaderSlotnames
    ]
