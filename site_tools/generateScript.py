...
def getTargetPath(prefix, suffix, env):
    ...
    if env.has_key('PackageName'):
        pkgname = env['PackageName']
    return os.path.join(prefix, targettype, pkgname, suffix)
...
def generateShellScript(scriptFile, env, wrapper):
    ...
    scriptFile.write('\n')
    scriptFile.write('#LD_LIBRARY_PATH=' + getTargetPath('$INST_DIR', env['LIBDIR'], env) + '\n')
    scriptFile.write('LD_LIBRARY_PATH=$INST_DIR/' + env['LIBDIR'])
    if env.has_key('WRAPPERLIBS'):
        for lib in env['WRAPPERLIBS']:
            scriptFile.write(':' + lib)
